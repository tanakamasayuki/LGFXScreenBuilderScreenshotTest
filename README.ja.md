...
このギャラリーは**大まかなデザインを一目で確認する**ためのもの。開発は次のループで進む。

1. **編集** — [LGFXScreenBuilder](https://github.com/tanakamasayuki/LGFXScreenBuilder)
   （ブラウザツール）で画面をデザインする。Chrome 系ブラウザ（Chrome / Edge）では
   File System Access API によりローカルファイルを直接上書きできるため、
   **プロジェクトの保存**や**ヘッダのエクスポート**がこのリポジトリの
   `Sfm.lgfxsb.json` / `Sfm.h` を直接書き換える（ダウンロード→コピーの手間がない）。
2. **画像生成** — いずれかの方法で:
   - ローカル — `uv run pytest screenshot/` で `docs/` を再生成、または
   - push — GitHub Actions が一覧を再生成してコミットし直す。更新されたファイル
     だけを `git pull` で取得して確認できるので便利。
3. **確認** — ギャラリー（プロファイル別／シーン別ページ）で大まかなデザインを確認する。
4. **詳細は別の手段で検証** — これらは静的なデザイン／プレビュー状態の
   レンダリングなので、テキストの差し替えや実行時に動的に描画されるものはここには
   出ない。実機、もしくはパソコン上の SDL2 `host` ディスプレイで確認する。
...
**なぜ LovyanGFX か。** キャプチャは
[LovyanGFX](https://github.com/lovyan03/LovyanGFX) で描画している。アプリが
**M5Unified / M5GFX** で描画していても描画 API に互換性があるため、ここでも
同じように描画される。さらに LovyanGFX はオフスクリーンデバイスを任意の解像度で
作成できるので、1 台のヘッドレスホスト上で全プロファイルをキャプチャするのに便利。
...
（と LovyanGFX）は `sketch.yaml` にバージョン指定されており、初回コンパイル時に
arduino-cli が自動インストールする。
...
ハーネスは生成ヘッダにしか依存しない（ビルドに必要な LGFXScreenBuilder
ライブラリは `sketch.yaml` にバージョン指定され自動インストールされる）ので、
...
push（および手動実行）で動作する。本リポジトリのみをチェックアウトし（プラットフォーム
とライブラリは初回コンパイル時に `sketch.yaml` から自動インストールされる）、
キャプチャ → `docs/` 再生成 → 一覧をコミットして戻す。
